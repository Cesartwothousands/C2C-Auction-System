...
public class AdminDao extends Dao {
    ...
    // return a list with length 2, the first is the member object, the second is
    // the total spending
    public List<Object> bestBuyer() {
        ...
        String buyerId = "select idUser,price from mydb.bid join (" + selledItems
                + ") as t on bid.idItem=t.idItem where bid.price=t.currentPrice";
        String buyerSpending = "select A.idUser,sum(price) as price from mydb.end_user join (" + buyerId
                + ") as A on end_user.idUser=A.idUser group by idUser";
